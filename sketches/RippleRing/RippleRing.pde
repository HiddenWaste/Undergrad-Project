...
void draw() {
  ...
  rotationAngle += map(currentFrequency, 110, 880, 0.001, 0.01);
}
...
class Ring {
  ...
  void update() {
    phase += currentFrequency/1000.0;
  }
  
  void display(color c, float mod) {
    noFill();
...
class Particle {
  ...
  void update() {
    ...
    vel.rotate(sin(frameCount * 0.1) * modulation * 0.1);
